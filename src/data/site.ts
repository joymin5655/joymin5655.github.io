    ...
    work: {
      ...
      cards: [
        ...
        },
        {
          ...
          blurb: '원자 노트 268 · typed edge 890 · orphan 0. 캡처(기계)와 증류(LLM)를 분리하고, 린트가 0이 아니면 세션을 끝낼 수 없는 개인 지식 인프라. bulk import 오염과 시크릿 사고에서 게이트를 배웠습니다.',
        ...
        },
        {
          ...
          blurb: 'FASTQ → 정렬 → 변이 → 주석 → 집단 빈도 → 우선순위 → 보고서. 계산과 로컬 LLM 추론을 분리하고, 지표를 수치 전에 동결해 FAIL도 그대로 보고했습니다. truth set 없는 도메인에서 불일치를 재는 법.',
        ...
        },
        {
          ...
          blurb: '프론트엔드 약 80% 주도, Collaboration상 — 그리고 배포 실패. 2026년 서버가 하던 일을 전부 브라우저(MediaPipe WASM + ONNX)로 옮겨 원본과 오차 0.000005로 일치함을 증명했습니다.',
        ...
        },
        {
          ...
          blurb: 'AI로 대량 제작하고 검증 하네스가 먼저 판정하는 방식. "깨진다"는 제보의 실체가 레이아웃이 아니라 대비였음을 87 스윕으로 먼저 배제했고, 모션은 라이브러리가 아니라 모델을 이식해 수치로 검증했습니다.',
    ...
    work: {
      ...
      cards: [
        ...
        },
        {
          ...
          blurb: '268 atomic notes · 890 typed edges · 0 orphans. Capture (machine) and distillation (LLM) are split, and no session ends with lint above zero. The gates were learned from a bulk-import pollution and a secret leak.',
          chips: ['LLM Wiki', 'typed edges', 'gitleaks fail-closed', 'Three.js'],
          links: [],
        ...
        },
        {
          ...
          blurb: 'FASTQ → alignment → variants → annotation → population frequency → prioritisation → report. Computation and local-LLM reasoning are separated, metrics were frozen before any number, and the FAILs are reported as they came. Measuring disagreement where no truth set exists.',
          chips: ['bcftools · freebayes', 'VEP · snpEff', 'ESM-2', 'llama.cpp'],
          links: [],
        ...
        },
        {
          ...
          blurb: 'Led ~80% of the frontend, won the Collaboration award — and failed to deploy. In 2026 I moved everything the server did into the browser (MediaPipe WASM + ONNX) and proved parity with the original within 0.000005.',
          chips: ['Django', 'MediaPipe', 'XGBoost → ONNX', 'onnxruntime-web'],
          links: [],
        ...
        },
        {
          name: 'Design · Web Craft',
          tag: 'FABLE 65p · ALL-IN-ONE IA · Wardenkit',
          blurb: 'Build at volume with AI, let a verification harness judge first. 87 sweeps ruled out layout before anyone touched it — the “broken design” was contrast — and motion was ported as a model, not a library, then verified numerically.',
      ...
      posture: {
        ...
        sections: [
          ...
          },
          {
            ...
            solution:
              'MediaPipe로 관절 랜드마크를 뽑고 코·눈·귀·어깨·손목 9개 관절의 거리·각도 201개를 특징으로 XGBoost가 5클래스를 분류하는 방식으로 팀과 합의했습니다. 랜드마크 기반이라 거리·조명·마스크에 견고했고, 실시간 판정이 가능했습니다(본인 확인 기준 85–90% @ 30 FPS).',
        ...
        decisions: [
          { choice: '랜드마크 거리·각도 특징 + XGBoost', over: '픽셀 입력 CNN', why: '거리·조명·마스크에 견고하고 CPU 실시간이 됩니다. 문제의 불변량이 관절 관계라면 표현을 거기에 맞추는 게 모델을 키우는 것보다 낫습니다.' },
